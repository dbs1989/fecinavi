<?php
	...
	foreach($avaliadores as $avaliador){
		...
		$pdf->MultiCell(0,20, $avaliador['nome'],0,"C",false);	
		$pdf->Ln(20);
		...
		if(!empty($certificado["'trabalhos'"])){
			getAvaliadorProjetos($avaliador['id_usuario']);
			
			$pdf->SetFont("arial", "", 12);
			$pdf->SetY(140);
			$pdf->SetX(400);
			$pdf->SetMargins(400,20,20,20);
			$texto = "";
			$pdf->Ln(10); 
			$pdf->MultiCell(0,20, "Trabalhos Avaliados:",0,"J",false);
			$cont = 0;
			foreach ($projetos as $projeto) {
				$cont++;
				$texto .= $cont.". ".$projeto['titulo']."\n";
			}
			$pdf->SetX(405);
			$pdf->MultiCell(0,20, $texto,1,"J",false);
			$pdf->Ln(10);
		}

    }        
	$pdf->Output("avaliadores.pdf", "I");
?>